...
int main(){
	...
	printf("Insira um número menor que zero para finalizar o cadastro de notas:\n\n");
	for(i = 0; i<80; i++){
		while(true){
			printf("Digite a nota do aluno nº%i: ", quantidadeNotas + 1);
			scanf("%i", &nota);
			fflush(stdin);
			if(nota > 10) printf("Valor digitado não encontrado, tente outro valor...\n");
				else break;
		}
		if(nota < 0) break;
		else if(nota <= 10){
	...
	for(i = 0; i<quantidadeNotas; i++){
		...
		// Esse laço está procurando o valor da nota em questão dentro do vetor de notas já cadastradas, se ele encontrar, então a nota em questão já existe e não precisa ser cadastrada novamente.
